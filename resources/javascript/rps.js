  ...
  if (computerSelection == playerSelection) {
    ...
    if (computerScore === 1) {
      ...
      displayResults(
        `${capitalize(
          computerSelection
        )} beats ${playerSelection}. It's not looking good, fam..`
      );
    } else if (computerScore === 4) {
      displayResults(
        `Your last shot, mate! ${capitalize(
          computerSelection
        )} beats ${playerSelection}. Lose and you become a whitewalker!`
      );
    } else {
    ...
    if (playerScore === 1) {
      ...
      displayResults(
        `${capitalize(
          playerSelection
        )} beats ${computerSelection}! One game wonder? Or you're going to save us all for real??`
      );
    } else if (playerScore === 4) {
      displayResults(
        `${capitalize(
          playerSelection
        )} beats ${computerSelection}. By the old gods and the new, if you win this you defeat The Night King!`
      );
    } else {
...
function declareWinner() {
  ...
  if (playerScore > computerScore) {
    endDesc.textContent = "GET IN! Mankind lives another day and the Iron Throne is yours!";
    returnMainBtn.innerText = "Play Again";
  } else {
    endDesc.textContent = "You lost, we're fucked.";
    returnMainBtn.innerText = "One Last Time?";
  }
  fadeIn();
